...
if "last_result" not in st.session_state:
    st.session_state["last_result"] = None  # {"answer": ..., "chunks": ...}
...
# ── 執行查詢 ──────────────────────────────────────────────────────────────────
if submit:
    # 讀取當下 textarea 的值（rerun 後仍正確）
    question = st.session_state.get("question_textarea", "").strip()
    if not question:
        st.warning("請輸入問題後再提問。")
    else:
        # 更新 session_state 問題文字，確保下次 rerun 仍顯示
        st.session_state["question_text"] = question
        st.session_state["last_result"] = None  # 重設舊結果
        ...
        try:
            ...
            if not chunks:
                ...
                # 儲存到 session_state，頁面 rerun 後仍能顯示
...
if result:
    ...
    st.markdown("---")
    with st.expander(f"📚 查看檢索到的 {len(result['chunks'])} 個來源段落", expanded=False):
